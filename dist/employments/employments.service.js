"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.EmploymentsService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const employments_entity_1 = require("./employments.entity");
let EmploymentsService = class EmploymentsService {
    constructor(EmploymentsRepository, dataSource) {
        this.EmploymentsRepository = EmploymentsRepository;
        this.dataSource = dataSource;
    }
    async findOne(id) {
        return await this.EmploymentsRepository.findOneBy({ id });
    }
    async count(job) {
        return await this.EmploymentsRepository.count({
            where: {
                job,
            }
        });
    }
    async calculateAverageRating(job) {
        const results = await this.dataSource.query(`select avg(p.rating) avg from jobs j join employments e on j.id = e.jobId and j.id = ?
    join posts p on p.id = e.postId
    group by j.id`, [job.id]);
        return Number(results[0].avg);
    }
    async createOne(employment, user, job, term) {
        employment.user = user;
        employment.job = job;
        employment.term = term;
        return await this.EmploymentsRepository.save(employment);
    }
};
EmploymentsService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(employments_entity_1.Employments)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.DataSource])
], EmploymentsService);
exports.EmploymentsService = EmploymentsService;
//# sourceMappingURL=employments.service.js.map